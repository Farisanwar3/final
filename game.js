(function(){
	...
	var mainState = {
		preload : function(){
			...
			this.game.load.image('bgSpace','assets/background1.png');
			this.game.load.image('bgSpace2','assets/starfield.png');
			this.game.load.spritesheet('ship','assets/lambo.png',245,120,1);
			"64,29,4"
			...
			this.game.load.audio('ak47', 'assets/ak47.mp3');
		},
		create1: function () {
			...
			this.timerEvent = this.timer.add(Phaser.Timer.MINUTE * 3 + Phaser.Timer.SECOND * 30, this.endTimer, this);
		...
		create : function(){
			...
			this.music3 = this.game.add.audio('ak47');
			this.numGabens = 0;
			// Create a custom timer
			...
			this.sprite = this.game.add.sprite(100, 200, 'ship');
			this.sprite.anchor.set(0.5);
			this.sprite.animations.add('move');
			this.sprite.animations.play('move', 2, true);
			...
			this.bulletsText = this.game.add.text(0,224,"Bullets : "+this.numBullets, style);
		},
		render: function () {
		...
		update : function(){
			...
			else if(this.game.input.keyboard.isDown(Phaser.Keyboard.RIGHT) && this.sprite.x < (WIDTH))
			{
			...
			//this increases the speed of the incoming sprites, such as patients, organs, and gabens
			if(this.game.input.keyboard.isDown(Phaser.Keyboard.V) && this.sprite.x > 0)
			{
				this.enemySpeed = 400;
				this.patientSpeed = 460;
				this.gabenSpeed = 175;
				this.patientsDeadSpeed = 490;
				this.fakeSpeed = 490;
				//this.sprite.body.velocity.x = 64*this.speed;
			}

			else if(this.game.input.keyboard.isDown(Phaser.Keyboard.C) && this.sprite.x > 0)
			{
				this.enemySpeed = 100;
				this.patientSpeed = 115;
				this.gabenSpeed = 175;
				this.patientsDeadSpeed = 112;
				this.fakeSpeed = 112;
			}
			else{
				this.enemySpeed = 200;
			...
			//If the player doesnt want to play anymore, then do this.
			...
			if(this.game.input.keyboard.isDown(Phaser.Keyboard.SPACEBAR))
			{
				if(curTime - this.lastBullet > this.rnd.integerInRange(145,300))
				{
					...
					if(curTime - this.lastFake > this.rnd.integerInRange(400,600))
					{
						this.generateFake();
			...
			if(curTime - this.lastPatient > this.rnd.integerInRange(999,1000))
			{
				this.generatePatients();
				this.lastPatient = curTime;
			}
			if(curTime - this.lastFake > this.rnd.integerInRange(600,700))
			{
				this.generateFake();
				this.lastFake = curTime;
			}
			if(curTime - this.lastPatientDead > this.rnd.integerInRange(999,1000))
			{
			...
			//Code works without enemyHitBullet defined
			this.game.physics.arcade.collide(this.enemies, this.bullets, this.enemyHitBullet,null, this);
			this.game.physics.arcade.collide(this.patientsDead, this.bullets, this.patientDeadHitBullet,null, this);
			this.game.physics.arcade.collide(this.patients, this.bullets, this.enemyHitBullet,null, this);
			this.game.physics.arcade.collide(this.fakes, this.bullets, this.enemyHitBullet,null, this);
			this.game.physics.arcade.collide(this.gabens, this.bullets, this.enemyHitBullet,null, this);
		...
		fireBullet : function(curTime){
			...
			if(bullet)
			{
				//took me a very long time to determine which numbers made sense so that the bullets shot from the center of the car
				bullet.reset(this.sprite.x+this.sprite.width-120,(this.sprite.y+this.sprite.height/2)-65);
		...
		generatePatientsDead : function(){
			...
			if(patientDead)
			{
				patientDead.reset(WIDTH - 10,Math.floor(Math.random()*(HEIGHT-60)),'patientcpy'+(1+Math.floor(Math.random()*5)));
				hit = false;
			}
			else
			{
				patientDead = this.patientsDead.create(WIDTH - 30,Math.floor(Math.random()*(HEIGHT-60)),'patientcpy'+(1+Math.floor(Math.random()*5)));
				hit = false;
			}
			
			patientDead.body.velocity.x = -this.patientsDeadSpeed;
			patientDead.outOfBoundsKill = true;

			patientDead.animations.add('move');
			patientDead.animations.play('move',1234,true);
		...
		fakeHitPlayer : function(player, fake){
			...
			this.score -= 75;
			this.numBullets= this.numBullets-5;
		...
		patientDeadHitBullet : function(bullet, patientDead){
			if(this.patientsDead.getIndex(patientDead) > -1)
				this.patientsDead.remove(patientDead);
			patientDead.kill();
			bullet.kill();
			this.numBullets++;
			this.numBullets= this.numBullets+5;
		...
		enemyHitPlayer : function(player, enemy){
			...
			this.score += 50;
			this.scoreText.setText("Score : "+this.score);
			this.numBullets= this.numBullets+30;
			this.bulletsText.setText("Bullets: "+this.numBullets);
		...
		gabenHitPlayer : function(player, gaben){
			...
			if(this.score >= 1000 && this.score <= 2000){
				//Had to change scores so that the severtiy in score reduction would be reduced
				//Used to be 625 here
				...
				//Used to be 250 here 
				//Had to change the score reduction because there was a negative score when I played the game
				this.score = this.score - 50;
			}
			this.numBullets= this.numBullets-50;
			this.bulletsText.setText("Bullets: "+this.numBullets);
			this.scoreText.setText("Score : "+this.score);
			//if(this.lives > 30)
				//this.game.state.start('menu');
		...
		patientHitPlayer : function(player, patient){
			...
			//this.title = this.game.add.text(250,170,"Patient transferred to organ transplant",style);
				...
				this.numOrgansText.setText("Organs collected : "+this.numOrgans);
				this.numDonated += 1;
				this.numDonatedText.setText("Patients passed and/or donated to: "+this.numDonated);
				this.score += 50;
	...
	var menuState = {
		...
		create : function(){
			...
			//speed used to be 10
			//this.speed = 10;this.ball.body.collideWorldBounds = true;
			this.speed = 5;
